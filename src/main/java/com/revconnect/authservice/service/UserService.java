...
import com.revconnect.authservice.repository.PasswordResetOtpRepository;
...
public class UserService {
    ...
    private static class PendingRegistration {
        ...
        private com.revconnect.authservice.enums.Handlers role;
    ...
    public String requestRegistrationOtp(RegisterRequest request) {
        ...
        com.revconnect.authservice.enums.Handlers role = request != null ? request.getRole() : null;

        if (email == null) return "EMAIL_REQUIRED";
        if (username == null || username.trim().isEmpty()) return "USERNAME_REQUIRED";
        if (role == null) return "ROLE_REQUIRED";
        if (password == null || password.length() < PASSWORD_MIN_LENGTH) return "PASSWORD_TOO_SHORT";
        if (!password.equals(confirmPassword)) return "PASSWORD_MISMATCH";
        if (userRepository.findByUsername(username.trim()) != null) return "USERNAME_EXISTS";
        if (userRepository.findByEmailIgnoreCase(email) != null) return "EMAIL_EXISTS";
    ...
    public String verifyRegistrationOtp(String email, String otp) {
        ...
        if (normalizedEmail == null) return "EMAIL_REQUIRED";
        if (otp == null || otp.trim().isEmpty()) return "OTP_REQUIRED";

        PendingRegistration pending = pendingRegistrations.get(normalizedEmail);
        if (pending == null) return "OTP_INVALID_OR_EXPIRED";
    ...
    public String verify(LoginRequestDTO request) {
        try {
            ...
            if (identifier.isEmpty()) return "FAIL";

            User existingUser = findByUsername(identifier);
            if (existingUser != null && !existingUser.isActive()) return "ACCOUNT_DEACTIVATED";
            ...
            if (authentication.isAuthenticated()) {
                ...
                if (authenticatedUser == null) return "FAIL";
    ...
    public String reactivateAccount(String usernameOrEmail, String password) {
        if (usernameOrEmail == null || usernameOrEmail.trim().isEmpty()) return "USERNAME_REQUIRED";
        if (password == null || password.isBlank()) return "PASSWORD_REQUIRED";

        User user = findByUsername(usernameOrEmail.trim());
        if (user == null) return "INVALID_CREDENTIALS";
        if (!passwordEncoder.matches(password, user.getPassword())) return "INVALID_CREDENTIALS";
        if (user.isActive()) return "ACCOUNT_ALREADY_ACTIVE";
    ...
    public User findByUsername(String username) {
        ...
        if (user == null) user = userRepository.findByEmailIgnoreCase(username);
        return user;
    }
    ...
    public String verifyPasswordResetOtp(String email, String otp) {
        ...
        if (user == null) return "OTP_INVALID_OR_EXPIRED";

        Optional<PasswordResetOtp> optionalOtp =
                passwordResetOtpRepository.findTopByUserAndUsedFalseOrderByCreatedAtDesc(user);
        if (optionalOtp.isEmpty()) return "OTP_INVALID_OR_EXPIRED";
    ...
    public String resetPasswordWithOtp(ResetPasswordRequestDTO request) {
        ...
        if (normalizedEmail == null) return "EMAIL_REQUIRED";
        if (otp == null || otp.trim().isEmpty()) return "OTP_REQUIRED";
        if (newPassword == null || newPassword.length() < PASSWORD_MIN_LENGTH) return "PASSWORD_TOO_SHORT";
        if (!newPassword.equals(confirmPassword)) return "PASSWORD_MISMATCH";
    ...
    public User getCurrentUser() {
        ...
        if (user == null) user = userRepository.findByEmailIgnoreCase(identifier);
        return user;
    }
    ...
    public String updateAccountDetails(String newUsername, String newEmail) {
        ...
        if (newUsername != null && !newUsername.equals(user.getUsername())) {
            if (userRepository.findByUsername(newUsername) != null) return "USERNAME_EXISTS";
            user.setUsername(newUsername);
        }
        if (newEmail != null && !newEmail.equals(user.getEmail())) {
            if (userRepository.findByEmailIgnoreCase(newEmail) != null) return "EMAIL_EXISTS";
    ...
    private void invalidateActiveOtps(User user) {
        ...
        if (!activeOtps.isEmpty()) passwordResetOtpRepository.saveAll(activeOtps);
    }
    ...
    private boolean sendOtpEmail(String email, String otp, String flowLabel) {
        ...
        try {
            ...
            helper.setText(buildOtpText(normalizedFlow, otp), buildOtpHtml(normalizedFlow, otp));
    ...
    private String resolveFromAddress() {
        if (mailFrom != null && !mailFrom.trim().isEmpty()) return mailFrom.trim();
    ...
    private String buildOtpHtml(String flowLabel, String otp) {
        ...
        return "<html><body style=\"margin:0;padding:0;background:#f4f7fb;font-family:Arial,sans-serif;\">"
                + "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" style=\"padding:24px 12px;\">"
                + "<tr><td align=\"center\">"
                + "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" style=\"max-width:560px;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;\">"
                ...
                + "<p style=\"margin:0 0 12px 0;font-size:15px;\">Use the OTP below to continue.</p>"
                + "<div style=\"margin:16px 0;padding:14px;border:1px solid " + safePrimary + ";border-radius:10px;"
                + "font-size:30px;letter-spacing:6px;font-weight:700;color:" + safePrimary + ";text-align:center;\">"
                + safeOtp + "</div>"
                + "<p style=\"font-size:13px;color:#475569;\">Valid for <strong>" + OTP_EXPIRY_MINUTES + " minutes</strong>.</p>"
